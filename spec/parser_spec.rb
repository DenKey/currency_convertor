...
describe Helpers::Parser do
  ...
  it 'raises errors when amount param is not a number' do
    expect do
      Helpers::Parser.new('USD', 'BAD', 'HKD').converted_amount
    end.to raise_error("Errors: 'BAD' is not a valid number")
  end

  it 'raises errors when currency param is unknown' do
    expect do
      Helpers::Parser.new('USD', 1000, 'BEB').converted_amount
    end.to raise_error("Errors: 'BEB' is not a valid currency")
  end
end
